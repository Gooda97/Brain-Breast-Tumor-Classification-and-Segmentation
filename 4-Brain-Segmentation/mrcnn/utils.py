...
def compute_iou_masks(masks1, gt_mask):
    masks1 = masks1[:, :, 0]
    gt_mask = gt_mask[:, :, 0]
    intersection = np.logical_and(masks1, gt_mask)
    union = np.logical_or(masks1, gt_mask)
...
def compute_dice(masks1, gt_mask):
    masks1 = masks1[:, :, 0]
    gt_mask = gt_mask[:, :, 0]
    intersection = np.logical_and(masks1, gt_mask)
    intersection_sum = np.sum(intersection)
    arr_sum = np.sum(masks1) + np.sum(gt_mask)
    dice = (2.0 * intersection_sum) / arr_sum
    return dice
