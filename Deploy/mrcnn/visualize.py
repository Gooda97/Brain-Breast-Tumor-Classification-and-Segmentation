...
def display_instances(
    ...
    if not N:
        ...
        # print(boxes.shape[0],masks.shape[-1],class_ids.shape[0],class_ids.shape)
    ...
    # Generate random colors
    colors = colors or random_colors(1)
    ...
    for i in range(1):
        ...
        if show_mask:
            ...
            # Add a third dimension of size 1 to make it a grayscale image
...
def display_top_masks(image, mask, class_ids, class_names, limit=1):
    ...
    mask_area = [np.sum(mask[np.where(class_ids == i)]) for i in unique_class_ids]
    top_ids = [
        v[0]
        for v in sorted(
    ...
    for i in range(limit):
        ...
        to_display.append(m)
        print(class_names, class_id, class_names[class_id] if class_id != -1 else "-")
        titles.append(class_names[class_id] if class_id != -1 else "-")
    display_images(to_display, titles=titles, cols=limit + 1, cmap="Blues_r")


def display_single_mask(image, mask, class_ids, class_names, limit=1):
    """Display the given image and the top few class masks."""
    titles = []
    titles.append("H x W={}x{}".format(image.shape[0], image.shape[1]))
    # Generate images and titles
    m = mask[:, :, np.where(class_ids == 1)[0]]
    m = np.sum(m * np.arange(1, m.shape[-1] + 1), -1)
    titles.append(class_names[class_ids[0]] if class_ids[0] != -1 else "-")
    display_images([image, mask], titles=titles, cols=limit + 1, cmap="Blues_r")
